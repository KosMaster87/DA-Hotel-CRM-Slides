...
```typescript
// Wir schreiben den Mechanismus selbst:
function renderHotels(hotels: Hotel[]): void {
  // getElementById findet das Element mit der ID 'hotel-list'
  // innerHTML ersetzt den Inhalt mit unserem HTML-String
  const list = document.getElementById("hotel-list")!;
  list.innerHTML = hotels
    .map((h) => `<li>${h.name} - ${h.stars} Sterne</li>`)
    .join("");
}
// React/Angular macht genau das - nur automatisch
```

> Entwickler die Vanilla JS können sind **deutlich bessere** Entwickler.
...
- **Viewer** (z.B. Rezeptionist) - nur lesen, kein Schreiben
- **Manager** (z.B. Hotelleiter) - Hotels, Buchungen, Kunden verwalten
- **Admin** - voller Zugriff + löschen
